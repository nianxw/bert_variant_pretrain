...
def fun(input_path, process_num_id):
    if process_num_id < 10:
        process_num_id = '0' + str(process_num_id)

    input_path = input_path + '%s' % str(process_num_id)

    output = open(input_path + '.variant', 'w', encoding='utf8')
    output_src = open(input_path + '.src', 'w', encoding='utf8')
...
if __name__ == "__main__":
    ...
    all_data_len = int(sys.argv[2])
    per_data_len = int(all_data_len / process_num)
    split_res = os.system('split -l %d %s -d -a 2 %s' % (per_data_len, input_path, input_path))

    process_list = []
    if split_res == 0:
        for process_num_id in range(process_num):
            p = Process(target=fun,args=(input_path, process_num_id)) #实例化进程对象
            p.start()
            process_list.append(p)
            print('启动进程%d' % process_num_id)
