...
def create_instances_from_document_nxw(all_documents, document_index, max_seq_length, short_seq_prob,
...
def create_training_instances(input_file, tokenizer, max_seq_len, short_seq_prob,
    ...
    for line_cnt, line in tqdm(enumerate(lines), desc='read data', total=len(lines)):
        line = line.strip()
    ...
    for document_index in tqdm(range(len(all_documents)), desc='create instances'):
        instances.extend(
            create_instances_from_document_nxw(
                all_documents, document_index, max_seq_len, short_seq_prob,
                max_ngram, masked_lm_prob, max_predictions_per_seq, vocab_words))
        # pbar(step=document_index)
    print('%s 数据处理完成' % input_file)
    ex_idx = 0
    while ex_idx < 5:
...
def main():
    ...
    parser.add_argument('--max_ngram', default=3, type=int)
    parser.add_argument("--do_data", default=True)
    parser.add_argument("--do_split", default=False)
    parser.add_argument("--do_lower_case", default=False)
    parser.add_argument('--seed', default=42, type=int)
    parser.add_argument("--line_per_file", default=2500000, type=int)
    # parser.add_argument("--file_num", type=int, default=1,
    #                     help="Number of dynamic masking to pregenerate (with different masks)")
    parser.add_argument("--max_seq_len", type=int, default=128)
    parser.add_argument("--short_seq_prob", type=float, default=0.1,
    ...
    if args.do_data:
        ...
        files = sorted([f for f in data_path.iterdir() if f.exists() and 'shard' in str(f)])
        for idx in range(len(files)):
            ...
            with save_filename.open('w') as fw:
                ...
                for instance in tqdm(file_examples, desc='save data'):
                    fw.write(instance + '\n')
                    fw.flush()
                    num_instances += 1
