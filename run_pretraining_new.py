...
from model.modeling_bert import BertForMaskedLM, BertConfig
...
class PregeneratedDataset(Dataset):
    def __init__(self, training_path, file_id, tokenizer, data_name, reduce_memory=False):
        ...
        with data_file.open() as f:
            for i, line in tqdm(enumerate(f), desc='Loading data: %s' % str(data_file)):
                # if iiii == 1000:
                #     break
                line = line.strip()
    ...
    def __getitem__(self, item):
        return (torch.tensor(self.input_ids[item].astype(np.int32)),
                torch.tensor(self.input_masks[item].astype(np.float32)),
                torch.tensor(self.segment_ids[item].astype(np.int32)),
...
def main():
    ...
    parser.add_argument("--vocab_path",default='/home/ssd6/nianxuanwei01/13_variant_pretrain/albert_pytorch_nxw/prev_trained_model/varaint_vocab.txt',type=str)
    parser.add_argument("--output_dir", default='/home/ssd6/nianxuanwei01/13_variant_pretrain/albert_pytorch_nxw/outputs', type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--model_path", default='/home/ssd6/nianxuanwei01/13_variant_pretrain/albert_pytorch_nxw/prev_trained_model', type=str)
    parser.add_argument('--data_name', default='bert', type=str)
    parser.add_argument("--file_num", type=int, default=105,
    ...
    init_logger(log_file=str(args.output_dir/ "train_bert_model.log"))
    ...
    for epoch in range(args.epochs):
        for idx in range(args.file_num):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                masked_lm_loss = loss_fct(prediction_scores.view(-1, len(tokenizer)), lm_label_ids.view(-1))
                loss = masked_lm_loss

                mask_metric(logits=prediction_scores.view(-1, len(tokenizer)), target=lm_label_ids.view(-1))
